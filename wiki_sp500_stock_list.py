...
""" This script grabs the Symbol, Company Name, and Industry for
    ...
    get_sp_500() automates the above functions and returns the output from
    get_sp_500_holdings()

"""


def get_wiki_page(url=None):
...
def get_sp_500_table(soup):
    table = soup.find('table', {'class': ['wikitable', 'sortable', 'jquery-tablesorter']})
    table_rows = table.find_all('tr')
    return table_rows

def get_sp_500_holdings(table_rows):
    sp_500_holdings = []
...
def get_sp_500():
    page = get_wiki_page()
    soup = soupify(page)
    sp_500_table = get_sp_500_table(soup)
    sp_500_holdings = get_sp_500_holdings(sp_500_table)
    return sp_500_holdings
